...
export interface AccountingService<T> {
    findAll(): Promise<T[]>;
    createOne(input: CreateCustomerInput | CreateProjectInput | CreateInvoiceInput): Promise<T>;
    findOne(id: string | number): Promise<T>;
    deleteOne(id: string | number): Promise<DeleteResult>;
    updateOne(id: string | number, input: UpdateCustomerInput | UpdateProjectInput | UpdateInvoiceInput): Promise<T>;
}
